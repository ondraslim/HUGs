...
namespace HUGs.Generator.DDD.IntegrationTests
{
    public class SourceGeneratorValidationTests : GeneratorTestBase
    {
        ...
        public void PropertyWhitelistedType_NoDiagnosticReported()
        {
            ...
            {
                foreach (var whitelistedType in DddPrimitiveType.PrimitiveTypes)
                {
                    ...
                    generatedFileTexts.Should().HaveCount(kind == "Enumeration" ? 2 : 3);
                }
            }
        }
        ...
        public void PropertyDddSchemaType_NoDiagnosticReported()
        {
            ...
            foreach (var kind in Kinds)
            {
                foreach (var kindOther in Kinds)
                {
                    ...
                    var expectedGeneratedFileCount = 5;
                    if (kind == "Enumeration") expectedGeneratedFileCount -= 1;
                    if (kindOther == "Enumeration") expectedGeneratedFileCount -= 1;
